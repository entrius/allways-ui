...
// what --from/--to take.
const role = (c: ChainInfo) =>
  c.hub ? '**hub**' : isAlpha(c.id) ? 'alpha' : '';
const assetRows = chains
  .map((c) => `| \`${c.id}\` | ${c.name} | ${role(c)} |`)
...
\`sn<N>\` is stake on subnet N. Send a \`sn<N>\` source as **one** \`transfer_stake\` (origin = destination netuid N, exact amount, one hotkey). Received alpha lands as stake under a miner-picked hotkey, not in your free balance. Details: ${alphaGuide}.
...
| \`sn<N>\` | The same coldkey; the CLI sends one \`transfer_stake\` from one hotkey that holds the exact amount |
