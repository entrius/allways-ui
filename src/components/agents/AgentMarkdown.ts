...
> \`6JVBEj5w27J2SVjERmv2c7wXgFee9nSSBKUJevHehyBD\` — the program id is the
...
- **Actors.** Miners post collateral and quote live rates per pair on-chain — they're active and quoting *before any swap exists*. Users pick a pair and an acceptable rate; that selection resolves on the backend to a specific miner who's offering it. Validators verify both legs of the swap and vote. Contract enforces slash / timeout / payout.
- **Reservation lifecycle.** Before a swap reaches \`ACTIVE\`, you go through \`reserve → confirm → initiate\`. A successful reserve locks a miner to you for the reservation TTL. Once you broadcast your source tx and validators verify it (on the source chain, correct sender, correct amount), the reservation graduates to an on-chain \`ACTIVE\` swap. If you don't confirm in time, the reservation expires, the miner unlocks, and nothing is lost — start over. View pre-initiate state via \`alw view reservation\` (yours) or \`GET /reservations/by-source/{address}\` (any).
...
- **Sender verification.** Validators reject any source tx whose on-chain sender does not match the address you proved at reserve time. Don't broadcast from any wallet other than the one tied to your reservation.
...
2. **Reserve.** You sign a proof of ownership of your source address and broadcast a \`SwapReserveSynapse\` to validators. Quorum of validators vote on-chain to lock the chosen miner to you for the **reservation TTL** (live value via \`alw view contract\`). \`alw swap now\` does this for you.
3. **Send source funds.** You broadcast a tx on the source chain — for TAO source, your **coldkey** signs and sends the transfer (your hotkey is *not* involved); for BTC source, the CLI signs with the WIF in \`.env\` or you broadcast from your own wallet. Either way, the source must be **the address you proved at reserve time** going to the miner's address, and it must happen inside the reservation TTL. As an Allways user (not a miner) you don't actually need a hotkey at all — bittensor wallets bundle one by convention but no swap step uses it on the user side.
...
Wallets land in \`~/.bittensor/wallets/\`. The hotkey is a Bittensor convention; as a swap user (not a miner) you don't actively use it on the happy path — see "Wallet roles & funding" below.

#### Wallet roles & funding (read this before funding anything)

The bittensor wallet bundles a coldkey and a hotkey. They do *different* things, and the doc has historically buried this — surface it now:

| Operation | Who signs / where funds come from |
|---|---|
| **TAO source transfer** (TAO → BTC swap, your sending leg) | **Coldkey** signs and the TAO debits from coldkey balance. |
| **BTC source transfer** (BTC → TAO swap, your sending leg) | Your BTC WIF (\`BTC_PRIVATE_KEY\`) or your own BTC wallet — no Bittensor key involved. |
| **Reserve / confirm proofs** (off-chain message signing) | **Coldkey** for TAO source addresses. No on-chain fee; no balance needed. |
| **Receiving destination asset** | The address you pass via \`--receive-address\` — TAO ss58 for \`--to tao\`, BTC \`bc1q…\` for \`--to btc\`. |
| **Auto-paid timeout refund** (slash) | Lands in \`swap.user\` (your registered source ss58 / coldkey for TAO source). Hotkey not involved. |
| **\`alw claim <swap-id>\`** (manual claim if auto-payout failed) | **Hotkey** signs the on-chain extrinsic; the funds still go to \`swap.user\` (coldkey). Hotkey only needs the ~0.02 TAO extrinsic-fee buffer (\`MIN_BALANCE_FOR_TX_RAO = 20_000_000\` rao). |
| **Miner ops** (collateral, activate, mark-fulfilled) | Hotkey signs and pays from hotkey balance — ignore unless you're running a miner. |

**Pre-flight funding checklist (per-direction, swap-only):**

- **TAO → BTC.** Top up the **coldkey** with at least \`amount + 0.02 TAO\` (swap amount + extrinsic fee buffer). Hotkey can be empty. \`--receive-address\` is your **BTC** address.
- **BTC → TAO.** Top up the BTC wallet whose WIF / address you'll broadcast from with at least \`amount + estimated BTC fee\`. Coldkey and hotkey can both be empty *for the swap itself* — though if you ever expect to need \`alw claim\`, fund the hotkey with ~0.02 TAO. \`--receive-address\` is your **TAO** ss58.
...
\`lightweight\` mode talks to public Esplora-compatible APIs (Blockstream as primary, with a Mempool.space fallback if Blockstream is unreachable) — no Bitcoin node required.

If \`BTC_PRIVATE_KEY\` is unset, BTC→TAO falls back to a manual flow: the CLI prints the miner's address + exact amount, you broadcast from your own wallet, **then run \`alw swap post-tx <tx-hash>\` *within the reservation TTL window* (run \`alw view contract\` for the current value)**. Miss the window and the reservation expires before you can confirm.

TAO→BTC swaps need no \`.env\` — your **coldkey** signs and sends the source TAO transfer directly.

## Verify the install + scope the system (read-only, no funds spent)

Run in order. Each step confirms the previous one. Steps 7–8 are the most important *before* attempting a swap — they tell you the live bounds and who's online.

    alw --help                                            # 1. Binary works; disclaimer prints.
    alw config                                            # 2. Saved config is correct (network, netuid, contract).
    alw status                                            # 3. Connects to chain; balances appear.
    alw view miners                                       # 4. Rows of miner UIDs + rates. Empty list = miners offline / wrong contract address.
    alw view validators                                   # 5. Whitelisted validator allowlist — confirms you're talking to a live validator set.
    alw view rates --pair btc-tao                         # 6. Live orderbook (only swappable miners).
    alw view contract                                     # 7. **Read this.** Live values for reservation TTL, fulfillment timeout, fee, min/max swap, extension caps. Use these instead of any number quoted in this doc.
    alw swap quote --from btc --to tao --amount 0.001     # 8. Preview a quote (no commitment). See output shape below.

\`alw swap quote\` example output (fields you'll parse):

    Pair:            BTC → TAO
    Source amount:   0.00100000 BTC
    Best rate:       1 BTC = 300.123456 TAO   (miner UID 12, hotkey 5Hax…)
    You receive:     0.297122 TAO             # ← post-fee receive amount
    Protocol fee:    1% (implicit in rate)

The "You receive" line is the post-fee amount that will actually land in your destination address — the 1% fee is implicit in the rate, not a separate charge. Always treat this number, not the headline rate, as the bottom-line outcome.

If any step fails, jump to **Known issues** below. In particular, an empty \`alw view miners\` while the dashboard shows active miners almost always means \`contract-address\` in your config doesn't match the live deployment — re-check it against the values in this doc.

## Run a swap

### Non-interactive (recommended for agents)

Drive the entire flow with flags — no prompts. \`--auto\` picks the best-rate eligible miner; \`--yes\` skips every confirmation. Read the flag table below before copying — every flag is meaningful and several have no safe default.

**BTC → TAO** (you send BTC; receive TAO):

    alw swap now \\
      --from btc --to tao \\                     # direction
      --amount 0.001 \\                          # source amount, in BTC
      --receive-address 5C...           \\       # destination — your TAO ss58 (where miner sends to)
      --from-address bc1q...            \\       # source — your BTC address (must match the wallet that broadcasts)
      --auto --yes

**TAO → BTC** (you send TAO; receive BTC). \`--receive-address\` here is a **BTC address**, not a TAO address — that's the destination chain:

    alw swap now \\
      --from tao --to btc \\                     # direction
      --amount 1.0 \\                            # source amount, in TAO
      --receive-address bc1q...         \\       # destination — your BTC address (where miner sends to)
      --from-address 5C...              \\       # source — your TAO coldkey ss58 (the coldkey signs the source transfer)
      --auto --yes

If you've already broadcast the source tx yourself (e.g. you skipped \`BTC_PRIVATE_KEY\` for BTC source), pass \`--from-tx-hash\` so the CLI doesn't try to send for you:
...
| Flag | Purpose |
|---|---|
| \`--from <chain>\` | Source chain (\`btc\`, \`tao\`) — the chain you spend from |
| \`--to <chain>\` | Destination chain — the chain you receive on |
| \`--amount <n>\` | **Source amount**, expressed in source-chain units. e.g. \`--from btc --amount 0.001\` = 0.001 BTC; \`--from tao --amount 1.0\` = 1 TAO. The matching help text in \`alw swap now --help\` calls this "Source amount". |
| \`--receive-address <addr>\` | Destination address — where the miner sends the destination asset to **you**. Must be on the \`--to\` chain (BTC \`bc1q…\` if \`--to btc\`; TAO ss58 if \`--to tao\`). |
| \`--from-address <addr>\` | Source address — where **you** broadcast the source tx from. Must be on the \`--from\` chain and match the wallet that signs (BTC \`bc1q…\` for \`--from btc\`; TAO **coldkey** ss58 for \`--from tao\`). Validators reject any source tx whose sender doesn't match this. |
| \`--from-tx-hash <hash>\` | Reuse an existing source tx — skip the CLI's send step (use when you broadcast manually). |
| \`--auto\` | Auto-pick the best-rate eligible miner — no menu. |
| \`--yes\` | Skip every confirmation prompt. |
| \`--btc-fee-rate <sat/vB>\` | Override BTC fee rate (default: auto-estimated to a next-block tier — see the BTC fee warning above). |
...
- **Reservation expired before send** — miner is auto-unlocked. Start a new swap; nothing is lost. Live reservation TTL is read via \`alw view contract\`.
...
Testnet parameters drift — always check \`alw view contract\` for current bounds before assuming, and run \`alw view miners\` / \`alw view validators\` to confirm the system is actually live before trying to swap.
