...
2. **Bid.** \`alw swap now\` — routed by default: the configured validator enters the pool for you (and fronts the entry fee). First bid pins the rate.
3. **Draw + finalize.** A validator cranks the draw; the winning router finalizes the seat with your amounts. Expect ~30–90s from bid to a live reservation (pool window + crank). If the CLI reports the pool didn't resolve in time, **re-running is safe and resume-aware** — no funds have moved. The live reservation then holds the miner exclusively for the reservation TTL.
...
Optionally set a WIF via env (shell env, a project \`.env\` walked up from CWD, or \`~/.allways/.env\`) — used by the CLI's local BTC signing, and by the auto-send flow once it lands:

    BTC_NETWORK=mainnet          # testnet4 for testnet
    BTC_PRIVATE_KEY=<your_WIF_key>

BTC access runs over public Esplora APIs — no Bitcoin node (override endpoints with \`BTC_ESPLORA_URLS\`).
...
\`env\` resolves the Bittensor network, Solana cluster, BTC network, netuid, **and the recommended router** together — swaps route through that validator by default (see "Native vs routed"). No program id or RPC to copy (the program id is baked in, same across clusters). Config persists at \`~/.allways/config.json\`; inspect every effective value + its source with \`alw config\`.
...
| \`--router <hotkey>\` | Route through this validator for this swap (overrides the \`router\` config) |
| \`--no-router\` | Self-represent even when a router is configured (native bid — loses contested draws) |
...
\`alw swap now\` is fully flag-driven — \`--from\`, \`--to\`, \`--amount\`, and \`--receive-address\` are mandatory (interactive prompts and auto fund-sending are on the roadmap). Missing flags fail fast with a clear message and a non-zero exit.

## CLI cheat sheet (real commands only)

Every read command (\`alw status\`, \`alw view …\`, \`alw swap quote\`) accepts \`--json\` for machine-readable output — errors come back as \`{"error": …}\` with a non-zero exit, so pipelines never parse prose.
...
| \`alw swap resume-reservation\` | Resume an interrupted flow — *deferred (exits 69); re-run \`alw swap now\` instead* |
...
| GET | \`/swaps/count\` | Total swap count |
| GET | \`/protocol/constants\` | Live contract parameters over HTTP (the \`alw view config\` equivalent) |
| GET | \`/sse\` | Server-Sent Events (channels: \`connected\`, \`event\`, \`miner\`, \`swap\`, \`reservation\`, \`head\`) |

This document, raw: \`https://all-ways.io/llms.txt\` (testnet mirror \`https://test.all-ways.io/llms.txt\`) — served by the dashboard hosts, not the API.
...
- **\`BTC signing requires the BTC_PRIVATE_KEY env var\`** — set the WIF for local BTC signing, or accept the manual flow (send from your own wallet, then \`alw swap post-tx <hash>\`).
