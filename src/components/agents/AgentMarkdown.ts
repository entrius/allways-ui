...
- **Rates.** A pair's hub leg (on an alpha↔spoke pair, its alpha leg) is its numéraire: the forward rate reads "destination per 1 of that leg", and the reverse leg inverts it. Collateral and swap bounds are denominated in the backing; the reservation fee is always SOL, because every bid settles on Solana.
...
- **Statuses.** \`PendingAttestation → Active → Fulfilled → Completed\` is the happy path. Two terminals end it early:
  - \`TimedOut\` — the miner failed to deliver. The protocol pays you **1.1× the swap's hub-leg value** (on an alpha pair with no TAO leg, the alpha leg's TAO value at the fill block), in the backing asset, automatically.
...
- **Live parameters — read, never hardcode.** Min/max swap bounds are **per backing** and are measured against the swap's hub leg (on an alpha pair with no TAO leg, the alpha leg's TAO value), not the source amount. They, the reservation fee, min/max collateral, reservation TTL, finalize window, fulfillment timeout, extension budget, and consensus threshold are all on-chain. Read them with \`alw view config\`.
