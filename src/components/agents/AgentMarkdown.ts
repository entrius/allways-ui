...
> **Where to point your wallets right now:** **testnet** (Bittensor netuid 19 +
> Solana devnet) is the live, usable environment — dry-run here. **Mainnet**
> (netuid 7) is in internal bring-up. Both use the same Solana program
> \`6JVBEj5w27J2SVjERmv2c7wXgFee9nSSBKUJevHehyBD\` — the program id is the same
> across clusters; only the network differs. Setup blocks for both are below.
...
For BTC-source swaps the CLI can broadcast BTC if you supply a WIF via env (shell env, a project \`.env\` walked up from CWD, or \`~/.allways/.env\`):

    BTC_MODE=lightweight
    BTC_NETWORK=mainnet
    BTC_PRIVATE_KEY=<your_WIF_key>

\`lightweight\` talks to public Esplora APIs — no Bitcoin node. If \`BTC_PRIVATE_KEY\` is unset, BTC-source falls back to manual: the CLI prints the miner's address + exact amount, you broadcast, then run \`alw swap post-tx <hash>\` immediately.

### 5. Configure the CLI

**Testnet (live — netuid 19):**
...
\`--yes\` skips confirmations. Read the flag table before copying.
...
If you already broadcast the source tx yourself, pass \`--from-tx-hash <hash>\` so the CLI doesn't try to send.
...
\`alw swap now\` with no flags walks through direction, miner pick, amount, address, bid, send, watch.
...
Testnet (Bittensor netuid 19 + Solana devnet) is the recommended place to dry-run. Configure:
